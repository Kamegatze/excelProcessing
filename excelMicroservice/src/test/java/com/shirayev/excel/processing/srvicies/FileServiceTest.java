...
import com.shirayev.excel.processing.dto.*;
...
public class FileServiceTest {
    ...
    void givenFile_whenWriteInDatabase_thenReturnFileDto() throws IOException {
        ...
        * */
        ...
        BDDMockito.given(excelParser.parse(ArgumentMatchers.any(InputStream.class))).willReturn(sheetsDtoList);
        BDDMockito.given(mapperClazzMock.getListObject(sheetsDtoList, Sheets.class)).willReturn(sheets);
        BDDMockito.given(fileRepository.save(ArgumentMatchers.any(File.class))).willReturn(file);
    ...
    @Test
    void givenFiles_whenGetFiles_thenReturnPageDtoWithFiles() {
        /*
        * given
        * */
        PageRequestDto pageRequestDto = new PageRequestDto();
        List<FileDto> fileDtoList = List.of(
                new FileDto(1L, "excel_without_title.xls"),
                new FileDto(2L, "excel-test.xls"),
                new FileDto(3L, "excel-other.xls"),
                new FileDto(4L, "excel-data.xls"),
                new FileDto(5L, "excel.xls")
        );
        List<File> files = mapperClazz.getListObject(fileDtoList, File.class);
        Page<File> page = new PageImpl<>(files, PageRequestDto.getPageRequest(pageRequestDto), files.size());

        BDDMockito.given(fileRepository.findAll(PageRequestDto.getPageRequest(pageRequestDto))).willReturn(page);
        BDDMockito.given(mapperClazzMock.getListObject(page.getContent(), FileDto.class)).willReturn(fileDtoList);

        /*
        * when
        * */
        PageDto<FileDto> pageDto = fileServiceImp.getFiles(pageRequestDto);
        ...
        * */
        ...
        Assertions.assertEquals(pageDto.getCountPage(), page.getTotalPages());
        Assertions.assertEquals(pageDto.getCountElements(), page.getTotalElements());
        Assertions.assertEquals(pageDto.getCurrentPage(), pageRequestDto.getPageNumber());
        Assertions.assertEquals(pageDto.getCountElementsInPage(), pageRequestDto.getPageSize());
        Assertions.assertEquals(fileDtoList.toString(), pageDto.getContent().toString());
    }

    @Test
    void givenFileNesting_whenGetFilesNesting_thenReturnPageDtoWithFileNesting() {
        List<FileNesting> fileNestingList = List.of(
                FileNesting.builder()
                        .id(1L)
                        .name("excel_without_title.xls")
                        .sheets(mapperClazz.getListObject(sheetsDtoList, SheetsNesting.class))
                        .build(),
                FileNesting.builder()
                        .id(2L)
                        .name("excel.xls")
        ...
        PageRequestDto pageRequestDto = new PageRequestDto();
        List<File> files = mapperClazz.getListObject(fileNestingList, File.class);
        Page<File> page = new PageImpl<>(files, PageRequestDto.getPageRequest(pageRequestDto), files.size());

        BDDMockito.given(fileRepository.findAll(PageRequestDto.getPageRequest(pageRequestDto))).willReturn(page);
        BDDMockito.given(mapperClazzMock.getListObject(page.getContent(), FileNesting.class)).willReturn(fileNestingList);

        PageDto<FileNesting> pageDto = fileServiceImp.getFilesNesting(pageRequestDto);
        ...
        Assertions.assertEquals(pageDto.getCurrentPage(), pageRequestDto.getPageNumber());
        Assertions.assertEquals(pageDto.getCountElementsInPage(), pageRequestDto.getPageSize());
        Assertions.assertEquals(fileNestingList.toString(), pageDto.getContent().toString());
    }
}
